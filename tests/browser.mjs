...
try {
  ...
  assert.match(await page.locator('#loan-summary').innerText(), /146\.337,00/);
  assert.match(await page.locator('.eligibility').innerText(), /6\.087,00/);
  assert.equal(await page.locator('.term-payment > strong').allTextContents().then(x => x.join('')), '—————');
  ...
  for (const width of [320, 390, 768, 1440]) {
    ...
    assert.equal(await page.locator('.setting-row').count(), 13);
